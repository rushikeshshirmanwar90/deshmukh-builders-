...
if (isset($_POST['update_project'])) {
    ...
    if (empty($id) || empty($name) || empty($desc) || empty($image)) {
        ...
        $sql = "UPDATE project SET name = '$name', des = '$desc', image = '$image', location = '$location', yt = '$yt', short = '$shortDesc', pdf = '$pdf'  WHERE id = '$id'";
...
<head>
    ...
    <title>Continue update project</title>
</head>
...
<body>
    ...
    <div class="container">
        <div class="admin-product-form-container center">
            <?php
            $select = mysqli_query($conn, "SELECT * FROM project WHERE id = '$id' ");
            while ($row = mysqli_fetch_assoc($select)) {
            ?>
                <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
                    <h3>Update <span style="color: #002245;"> CONTINUE </span> Project</h3>
                ...
                text-transform: capitalize;" for="">Update pdf</label>
                    <input type="file" required  pdf" name="pdf" class="box">


                    <input type="submit" required class="btn" name="update_project" value="Update Project">
                    <a href="continueProject.php"><input type="button" required class="btn" value="go Back"></a>
                </form>
        </div>
    </div>
